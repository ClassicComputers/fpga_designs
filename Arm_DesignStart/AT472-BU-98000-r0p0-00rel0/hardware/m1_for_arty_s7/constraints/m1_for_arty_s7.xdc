...
#
#  Purpose :    Constraints for M1 on Arty S7 board
#
...
# DDR IO is on bank 34, which then has to be SSTL135
# However one of the LED outputs is connected to the VREF pin.  So need to set
# the device to use it's own internal VREF
set_property INTERNAL_VREF 0.675 [get_iobanks 34]
# Optionally
# INTERNAL_VREF_BANK34 = 0.675
...
# Shield pin 40, (DAPLink[14]), is the Serial Wire data, which can be tristate.  Also needs a pull-up
...
create_generated_clock -name cclk -source        [get_pins -hier -regexp m1_for_arty_s7_i/axi_quad_spi_0/.*STARTUP2_7SERIES_inst/USRCCLKO] \
...
#set_clock_groups -name async_group -asynchronous -group {cpu_clk} \
                                                 -group {dap_qspi_clk dap_spi_clk cclk} \
                                                 -group {SWCLK} \
                                                 -group {slow_out_clk}
...
# Where not able to correctly specify the launch flop as -from, so use the resultant net with -through
...
set_input_delay  -clock [get_clocks cclk] -max -add_delay $qspi_id_Tco [get_ports qspi_flash_io?_io]
set_input_delay  -clock [get_clocks cclk] -min -add_delay $qspi_id_Tho [get_ports qspi_flash_io?_io]
set_input_delay  -clock [get_clocks cclk] -max -add_delay $qspi_id_Tco [get_ports qspi_flash_ss*]
set_input_delay  -clock [get_clocks cclk] -min -add_delay $qspi_id_Tho [get_ports qspi_flash_ss*]

set_output_delay -clock [get_clocks cclk] -max -add_delay $qspi_od_Tsu [get_ports qspi_flash_io?_io]
set_output_delay -clock [get_clocks cclk] -min -add_delay $qspi_od_Thd [get_ports qspi_flash_io?_io]
set_output_delay -clock [get_clocks cclk] -max -add_delay $qspi_od_Tsu [get_ports qspi_flash_ss*]
set_output_delay -clock [get_clocks cclk] -min -add_delay $qspi_od_Thd [get_ports qspi_flash_ss*]
