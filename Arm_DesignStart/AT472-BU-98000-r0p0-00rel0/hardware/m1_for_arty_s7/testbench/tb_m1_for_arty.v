...
//-----------------------------------------------------------------------------
// Purpose :
//     Testbench for the M1 on Arty S7 board example design
//-----------------------------------------------------------------------------
...
`endif
    ...
    // For increased accuracy run baud rate divider on both clock edges.
    localparam BAUD_RATE       = 115200;
//    localparam CLK_DIV_BAUD    = (BAUD_RATE)/22);     // 11 bits - 1 start, 8 data, 1 stop, 1 parity
    /* Calculate baud rate divider.  Input clock is 12MHz.
       Divider is (2* SYS_CLK_FREQ) BAUD_RATE = (2 * 12M)/115200 = 208
       However empirical measurement from simulation suggests that 207 is a more accurate value
    */
    localparam CLK_DIV_BAUD    = 207;
...
`ifdef INCLUDE_DAPLINK
    `ifndef DAPLINK_LINK_NF
        localparam SW_TOGGLE_RATE = 10000;
    `else
        localparam SW_TOGGLE_RATE = 120;
    `endif
`else
    localparam SW_TOGGLE_RATE = 120;
`endif
...
`ifdef ARTY_S7_TARGET
    m1_for_arty_s7_wrapper DUT (
`else
    m1_for_arty_a7_wrapper DUT (
`endif
        // Clocks and reset
        ...
        // S7 device outputs clock on CCLK pin, so not on an FPGA IO pin
`ifndef ARTY_S7_TARGET
...
`ifdef INCLUDE_QSPI_MODEL
    ...
    // S7 device outputs clock on CCLK pin, so not on an FPGA IO pin
    // Need to create an equivalent pin for simulation purposes
`ifdef ARTY_S7_TARGET
    wire ext_spi_clk = DUT.m1_for_arty_s7_i.ext_spi_clk;
    reg  base_qspi_clk;
    reg  qspi_flash_ss_io_d;
    
    always @(posedge ext_spi_clk)
        qspi_flash_ss_io_d <= qspi_flash_ss_io;
        
    always @(posedge ext_spi_clk)
        if( qspi_flash_ss_io || qspi_flash_ss_io_d)
            base_qspi_clk <= 1'b0;
        else
            base_qspi_clk <= ~base_qspi_clk;
            
    assign qspi_flash_sck_io = base_qspi_clk;
    
    // REVISIT - Current simulation of S7 base qspi fails.
`endif
